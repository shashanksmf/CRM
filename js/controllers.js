...
function MainCtrl($scope, API, $rootScope, crmconfig, $timeout, $interval) {
  ...
  $rootScope.$on('unauthorized', function(response) {
    ...
    $rootScope.config.responseText = "Please login again";
  });
  /**
  ...
  $scope.sendMessage = function(chatArrIndex, msg, toUserId) {
    ...
    API.sendMessage(sendMsgObj).then(function(response) {
      ...
      if (response.data.responce) {
        $rootScope.chats[chatArrIndex].chatDetail.push({
          id: response.data.msgId,
          message: msg,
          fromId: sendMsgObj.from,
          toId: toUserId,
          readed: false
        });
        ...
        $rootScope.chats[chatArrIndex].chatDetail.push({
          message: msg,
          from: sendMsgObj.fromId,
          toId: toUserId,
          readed: false,
          isError: true,
          errorMsg: 'MessageCould not be send..'
        });
      }
    })
  ...
  $scope.openChatBox = function(fromUserId, messageId, fromUserName) {
    ...
    var isFromIdFound = null,
      fromIdInChatArr = null;
    ...
    for (var i = 0; i < newMsgDataArr.length; i++) {
      for (var j = 0; j < $scope.chats.length; j++) {
        if ($scope.chats[j].chatDetail.length > 0) {
          ...
          if ($scope.chats[j].fromUserId == newMsgDataArr[i].fromId && $scope.chats[
              j].active) {
            if (parseInt(newMsgDataArr[i].id) > parseInt($scope.chats[j].chatDetail[
                chatDetailLength].id)) {
              $scope.chats[j].chatDetail.push(newMsgDataArr[i]);
              API.readMsg(newMsgDataArr[i].id, newMsgDataArr[i].fromId).then(
                function(response) {
                  console.log("response : ", response);
                });
            }
    ...
    API.getUserInfo(userId).then(function(response) {
      if (response.data.result) {
        ...
        $rootScope.userProfilePic = crmconfig.serverDomainName + "/" +
          response.data.details.profilePic;
      }
    });
  }
  ...
  $scope.hideFriendList = function() {
    var hasClass = angular.element(document.querySelector('.chatboxContainer'))
      .hasClass('hideFriendList');
    if (!hasClass) {
      angular.element(document.querySelector('.chatboxContainer')).addClass(
        'hideFriendList');
    } else {
      angular.element(document.querySelector('.chatboxContainer')).removeClass(
        'hideFriendList');
    }
  }
   ...
   */
  this.BarChart = {
    data: [5, 3, 9, 6, 5, 9, 7, 3, 5, 2, 4, 7, 3, 2, 7, 9, 6, 4, 5, 7, 3, 2,
      1, 0, 9, 5, 6, 8, 3, 2, 1
    ],
    options: {
      fill: ["#1ab394", "#d7d7d7"],
  ...
  this.LineChart = {
    data: [5, 9, 7, 3, 5, 2, 5, 3, 9, 6, 5, 9, 4, 7, 3, 2, 9, 8, 7, 4, 5, 1,
      2, 9, 5, 4, 7
    ],
    options: {
      fill: '#1ab394',
...
function sparklineChartCtrl() {
   ...
   * Long line chart
   */
  var longLineData = [34, 43, 43, 35, 44, 32, 15, 22, 46, 33, 86, 54, 73, 53,
    12, 53, 23, 65, 23, 63, 53, 42, 34, 56, 76, 15, 54, 23, 44
  ];
  var longLineOptions = {
    type: 'line',
    lineColor: '#17997f',
   ...
   * Discrate chart
   */
  var discreteData = [4, 6, 7, 7, 4, 3, 2, 1, 4, 4, 5, 6, 3, 4, 5, 8, 7, 6, 9,
    3, 2, 4, 1, 5, 6, 4, 3, 7,
  ];
  var discreteOptions = {
    type: 'discrete',
   ...
   * Bar chart
   */
  var barCustomData = [5, 6, 7, 2, 0, 4, 2, 4, 5, 7, 2, 4, 12, 14, 4, 2, 14, 12,
    7
  ];
   ...
   * Line chart
   */
  var lineCustomData = [34, 43, 43, 35, 44, 32, 15, 22, 46, 33, 86, 54, 73, 53,
    12, 53, 23, 65, 23, 63, 53, 42, 34, 56, 76, 15, 54, 23, 44
  ];
...
function CalendarCtrl($scope) {
  ...
  $scope.alertOnDrop = function(event, dayDelta, minuteDelta, allDay,
    ...
    $scope.alertMessage = (event.title + ': Droped to make dayDelta ' +
      dayDelta);
  };
  /* message on Resize */
  $scope.alertOnResize = function(event, dayDelta, minuteDelta, revertFunc,
    jsEvent, ui, view) {
    $scope.alertMessage = (event.title + ': Resized to make dayDelta ' +
      minuteDelta);
  };
