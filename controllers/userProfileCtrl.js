...
inspinia.controller('userProfileCtrl', ['$scope', '$rootScope', '$http', '$q',
	'API', '$state', '$timeout', '$stateParams', 'crmconfig',
	function($scope, $rootScope, $http, $q, API, $state, $timeout, $stateParams,
		...
		$scope.submit = function() {
			$scope.userProfileInfo.imgUrl = $scope.profilePicUploadedResUrl || $scope
				.userProfileInfo.imgUrl;
		...
		$scope.uploadNewFileAttach = function(fileName) {
			...
			uploadEmplFile($scope.fileAttach).then(function(response) {
				...
				$timeout(function() {
					$scope.files.push({
							checked: false,
							"name": response.details.fileName,
							"date": response.details.date,
							id: response.details.id,
							filesize: response.details.filesize,
							isactive: '1'
						})
